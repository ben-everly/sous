...
- **DB-touching commands serialize across worktrees.** `db:start`, `db:reset`, `db:stop`, `test:db` (pgTAP), and `test:e2e` operate on the shared stack, so each takes an exclusive lock — a concurrent run (from any worktree, or the same one) waits rather than clobbering it. Locking needs `flock`, present on Linux and CI; on stock macOS it's absent — `brew install flock` to enable it. Without `flock` these commands refuse to run, since an unserialized run can corrupt the shared stack; if you use only a single worktree, set `ALLOW_UNSERIALIZED_SUPABASE=1` to run without locking. `test:unit` is safe regardless — Supabase is mocked.
