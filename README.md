...
- **DB-touching commands serialize automatically.** `db:reset`, `test:db` (pgTAP), and `test:e2e` hit the shared database, so each takes a shared lock — a run from a second worktree waits rather than clobbering it. (Serialization needs `flock`, present on Linux and CI; on stock macOS it's absent, so they run unserialized with a warning.) `test:unit` is safe regardless — Supabase is mocked.
