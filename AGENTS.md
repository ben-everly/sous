...
- **RPC writes**: when invariants (state guards, authz) live in a `security-invoker` RPC, `useInsertMutation`/`useUpdateMutation` can't drive it — their no-op/failure semantics live in a table predicate cache-helpers can infer, not in an RPC body. Wrap the RPC in a plain `useMutation` with the standard optimistic lifecycle: `onMutate` awaits `queryClient.cancelQueries({ queryKey })`, snapshots `getQueryData(queryKey)`, then writes the optimistic row via the imperative by-PK helpers (`upsertItem` for an update/restore-shaped call, `deleteItem` for a destructive one), projecting the RPC's shape down to the cached columns (`COLUMNS` in `queries.ts`); `onError` writes the snapshot back and toasts; `onSettled` calls `invalidateQueries({ queryKey })` to reconcile to server truth regardless of outcome.
